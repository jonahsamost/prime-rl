...
class NIXLWeightUpdateWorker(Worker):
    ...
    def init_broadcaster(
        ...
        del inference_world_size, quantize_in_weight_transfer, delta_mode, delta_cuda_graphs
        ...
        self.transfer_representation = delta_representation
        self.fp8_scale_format = delta_fp8_scale_format
    ...
    def initialize_transfer(self) -> WeightTransferPlan:
        ...
        if table.representation != self.transfer_representation:
            raise RuntimeError(
                f"trainer published {table.representation!r} tensors but inference expects "
                f"{self.transfer_representation!r}"
            )
        expected_scale_format = self.fp8_scale_format if self.transfer_representation == "fp8_kernel" else ""
        ...
        self.validate_transfer_plan(plan)
    ...
    def update_weights_from_path(self, weight_dir: str | None = None) -> None:
        ...
        try:
            if policy.kind != "full":
                raise RuntimeError(f"NIXL full-transfer worker cannot apply policy kind {policy.kind!r}")
